...
<div class="text-slate-900" style="font-family: 'Times New Roman', serif;">
    <div class="mb-6">
        <h1 class="text-3xl font-bold">{{ $fullName }}</h1>
        <p class="text-sm text-slate-700">{{ $email }} @if($contact) | {{ $contact }} @endif</p>
        <p class="text-sm text-slate-700">{{ $profile->tp_number ?? '' }} @if($profile?->course_name) | {{ $profile->course_name }} @endif</p>
    </div>

    <section class="mb-6">
        <h2 class="text-base font-semibold uppercase tracking-wider text-slate-700">Summary</h2>
        <div class="mt-2 border border-slate-200 rounded-md p-3">
            @if ($summary)
                <p class="text-sm text-slate-700">{{ $summary }}</p>
            @else
                <p class="text-sm text-slate-500">Add your summary in Student Profile.</p>
            @endif
        </div>
    </section>

    <section class="mb-6">
        <h2 class="text-base font-semibold uppercase tracking-wider text-slate-700">Education</h2>
    ...
    <section class="mb-6">
        <h2 class="text-base font-semibold uppercase tracking-wider text-slate-700">Projects</h2>
    ...
    <section>
        <h2 class="text-base font-semibold uppercase tracking-wider text-slate-700">Skills</h2>
        <div class="mt-3 flex flex-wrap gap-2 border border-slate-200 rounded-md p-3">
            @forelse ($user->skills as $skill)
                <span class="text-xs text-slate-700 border border-slate-300 rounded-full px-3 py-1">
