@php
    ...
    // Notice the ?-> operator. If $profile is null, it instantly safely returns null.
    ...
    // The safely guarded address line
...
<div class="bg-white shadow-2xl w-full max-w-[21cm] min-h-[29.7cm] p-10 mx-auto text-gray-900 font-serif">
    ...
    <header class="text-center pb-5 border-b border-gray-300 mb-5">
        ...
        <div class="text-[11px] text-gray-700 mt-2 space-y-0.5">
            <p>{{ $address }}</p>
            <p>{{ $phone }} • {{ $email }} @if($profile?->linkedin_url) • <a href="{{ $profile->linkedin_url }}">{{ Str::after($profile->linkedin_url, 'linkedin.com/in/') }}</a> @endif</p>
        </div>
    </header>
